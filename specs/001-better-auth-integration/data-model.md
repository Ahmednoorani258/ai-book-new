...
This data model is based on the inferred schema requirements from the "Better Auth" library documentation. The library is responsible for managing these tables, likely through its CLI tools (`npx @better-auth/cli generate/migrate`).
...
-   **User has many Accounts**: One User can have multiple authentication accounts (e.g., email/password, Google, GitHub).
...
-   **Tokens**: Time-limited validity (e.g., `resetPasswordTokenExpiresIn` configurable, default 3600 seconds).
